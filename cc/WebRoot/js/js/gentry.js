...
function GentryList(list){
	if(list!=undefined){
	    ...
	    for(var i=0;i<list.length;i++){
	    	html=html+	    	
	    	"<li class=\"selected\">"+
	        "<span onclick=\"GentryID("+list[i].id+")\"><img style=\"height:126px;width:168px;\" src=\"http://localhost/"+list[i].photo1+"\" /></span>"+
	        "<h2><a>"+list[i].gentryname+"</a></h2>"+
	        "<p><a >价格：</a>&nbsp;<a >"+list[i].minprice+"&yen</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick=\"GentryID("+list[i].id+")\">查看</a>&nbsp;&nbsp;&nbsp;&nbsp;<a  class=\"click1\" onclick=\"GentryOrderShow("+list[i].id+")\">购买</a></p>"+			       
	        "</li>";	   	      
	...
	var urltext = "getGentryList1.json";
	$.ajax({
	        type: "POST",
...
function GentryID(id){
	...
	"<li><label class=\"label1\">文创名称：</label><label class=\"label1\" style=\"width:200px;\">"+data.gentryname+"</label></li>"+
	"<li><label class=\"label1\">价格：</label><label class=\"label1\" style=\"width:200px;\">"+data.minprice+"&yen</label></li>"+
	"<li><label class=\"label1\">提供商：</label><label class=\"label1\" style=\"width:200px;\">"+data.manufacturer+"</label></li>"+
	"<li><label class=\"label1\" style=\"float: inherit;\">文创描述：</label><div class=\"div1\">&nbsp;&nbsp;&nbsp;&nbsp;"+data.content+"</div></li>";
...
function GentryOrderShow(id){
	...
	var html="<li><label>文创名称：</label><input style=\"width: 245px;border-style:none;\" readonly=\"true\"  type=\"text\" class=\"dfinput\" value=\""+data.gentryname+"\" /></li>"+ 
	"<input type=\"text\" style=\"display:none;\" id=\"id\" class=\"sure\" value=\""+data.id+"\" />"+	 
	 ...
	 "<li><label>文创价格：</label><input id=\"minprice\" style=\"width: 45px;border-style:none;\" readonly=\"true\"  type=\"text\" class=\"dfinput\" value=\""+data.minprice+"\" />元</li>"+ 	
	...
	document.getElementById("tijiao").innerHTML="<input type=\"button\"  class=\"sure1\" value=\"提交订单\" onclick=\"GentryOrderAdd()\"/>";
}

//添加商城文创订单
function GentryOrderAdd(){
	...
	var urltext = "addGentryOrder.json";
	$.ajax({
	        type: "POST",
	        contentType:'application/x-www-form-urlencoded; charset=UTF-8',
	        url: urltext, 
